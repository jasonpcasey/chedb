...
nsf_institutions <- read_db("cdw", "SELECT unitid, medical_school FROM nsf_bridge") %>%
  group_by(unitid) %>%
...
institutions <- ic_meta %>%
  ...
  institutions %>%
    write_db("cdw", "ipeds_institutions")
}
...
if (params$read_raw == "N") {
  system.time({
    institutions <- read_db("cdw", "SELECT * FROM dbo.ipeds_institutions")
    })
}
