...
# Create Date: 20MAY2015
# Description: Reader script for IPEDS Completions, AY2001-2002
# Notes:
...
infile <- "http://nces.ed.gov/ipeds/datacenter/data/C2002_A.zip"
outfile <- "output-data/completions/c_2002_dat.csv"
...
# Read the data
dat <- read.table(file=unz(temp, 'c2002_a.csv'),
                  header=TRUE,
                  sep=",",
                  quote="\"",
                  skip=0,
...
names(dat)[names(dat) == 'cipcode'] <- c('CipCode')
...
c <- melt(dat, id.vars=c('Unitid','DataYear','CipCode','MajorNumber','AwardLevel'), variable.name='Variable',value.name='Awards')  
...
# Add NDCip Field
c$NdCipCode <- c$CipCode

# Remove empty fields and totals
c <- subset(c, Awards > 0 & CipCode < 99, select=c("Unitid","DataYear","CipCode","MajorNumber","AwardLevel","RaceEthnicity","Sex", "Awards"))
