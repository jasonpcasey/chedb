...
herd_details <- herd %>%
  ...
  mutate(funding_type = recode(as.integer(rownum),
                ...
                `11`="Non-federal")) %>%
  rename(unitid = unitid_xwalk) %>%
  select(inst_id, ncses_inst_id, unitid, year_id, funding_type, agency_id = column, academic_field_id, expenditure = data)  %>%
  group_by(inst_id, ncses_inst_id, unitid, year_id, funding_type, agency_id, academic_field_id) %>%
